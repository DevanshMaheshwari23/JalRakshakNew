...
/// Camera-only media capture with GPS metadata extraction
Future<Map<String, dynamic>?> capturePhotoWithMetadata({
  ...
  try {
    ...
    if (permission == LocationPermission.deniedForever) {
      _showErrorSnackBar(context, 'Location permissions permanently denied');
      return null;
    }
    ...
    // Capture photo using camera only
    final picker = ImagePicker();
    final pickedMedia = await picker.pickImage(
    ...
    final path = _getStoragePath(storageFolderPath, pickedMedia.name, false);

    // Get dimensions if requested
    MediaDimensions? dimensions;
    if (includeDimensions) {
      dimensions = await _getImageDimensions(mediaBytes);
    }
    ...
    _showErrorSnackBar(context, 'Error capturing photo: ${e.toString()}');
    return null;
  }
}
...
Future<List<SelectedFile>?> selectMediaWithSourceBottomSheet({
  ...
  // If camera only mode, use the new capture function
  if (cameraOnly && allowPhoto && !allowVideo) {
    final result = await capturePhotoWithMetadata(
      context: context,
  ...
  // Original gallery/camera selection logic
